...
== Controller (Kommunikation mit Frontend)
    GET mit id // Ausgabe eines einzelnen Elements
    GET mit userId // Ausgabe aller Elemente eines Users   
    Put mit id // Aktualisierung eines Elements
    Post // Erstellen eines neuen Elements
    Delete mit id // Löschen eines Elements
    wenn nicht dann error 405
...
=== Controller: categories
    ...
    Schema: `CreateCategorySchema` (zod) — validiert Body für POST (name, type, userId, icon, color)

    Exported handler: default async (event)

    Funktionen / Verhalten:
        - Handler(event): Dispatch je nach HTTP-Methode
        - `GET /api/categories?userId=:userId`: Validiert `userId` → ruft `CategoryService.getCategoryByUserId(userId)` → gibt Kategorie-Liste zurück
        - `GET /api/categories/:id`: Liest `id` aus Route → ruft `CategoryService.getCategoryByCategoryId(id)` → gibt einzelne Kategorie zurück
        - `POST /api/categories`: Liest Body, validiert mit `CreateCategorySchema` → baut `payload` → ruft `CategoryService.createCategory(payload)` → gibt erstellte Kategorie zurück
        - `PUT /api/categories/:id`: Liest `id` + Body (teilweise validiert) → ruft `CategoryService.updateCategory(id, data)` → gibt aktualisierte Kategorie zurück
        - `DELETE /api/categories/:id`: Ruft `CategoryService.deleteCategory(id)` → gibt Erfolgsmeldung zurück
        --> UserId und id seperat in zwei verschiedenen Endpunkten behandeln!
        - Andere Methoden → `405`
        - Fehlerbehandlung: Log + `createError`

    Aufruft / Abhängigkeiten:
        - `CategoryService.getCategoryByUserId(Number(userId))`
        - `CategoryService.getCategoryByCategoryId(id)`
        - `CategoryService.createCategory(payload)`
        - `CategoryService.updateCategory(id, data)`
        - `CategoryService.deleteCategory(id)`

    Rückgabewerte:
        - `GET /api/categories`: Array von Kategorien (vereinfachtes DTO: id, name, type, icon, color)
        - `GET /api/categories/:id`: einzelne Kategorie-DTO oder 404
        - `POST /api/categories`: DTO der erstellten Kategorie
        - `PUT /api/categories/:id`: DTO der aktualisierten Kategorie
        - `DELETE /api/categories/:id`: { message: 'Category deleted', id }
...
=== Controller: expenses
    ...
    Funktionen / Verhalten:
    - Handler(event):
    - `GET`: Validiert `userId` → ruft `ExpenseService.getExpensesByUserId(userId)` → gibt Liste der Ausgaben zurück
    - `GET /api/expenses/:id`: Liest `id` aus Route → ruft `ExpenseService.getExpenseById(id)` → gibt einzelne Ausgabe zurück oder 404
    - `POST`: Liest Body, validiert gegen `CreateExpenseSchema` → baut `payload` → ruft `ExpenseService.createExpense(payload)` → gibt Ergebnis zurück
    - `PUT /api/expenses/:id`: Liest `id` + Body (teilweise validiert) → ruft `ExpenseService.updateExpense(id, data)` → gibt Ergebnis zurück
    - `DELETE /api/expenses/:id`: Ruft `ExpenseService.deleteExpense(id)` → gibt Erfolgsmeldung zurück
    --> UserId und id seperat in zwei verschiedenen Endpunkten behandeln!
    - Andere Methoden → `405`
    - Fehlerbehandlung: Log + `createError`
    ...
    Rückgabewerte:
    - GET /api/expenses: Array von Expense-Domain-Objekten (vom Service transformiert)
    - GET /api/expenses/:id: einzelnes Expense-Objekt oder 404
    - POST /api/expenses: Objekt { message, expense }
    - PUT /api/expenses/:id: Objekt { message, expense }
    - DELETE /api/expenses/:id: Objekt { message, id }

    Zusätzliche Hinweise:
    - `GET /api/expenses` unterstützt Filtering (z.B. Datum, Kategorie) und Paging — Parser im Controller
    - `POST/PUT` validieren Body mit `CreateExpenseSchema` bzw. Partial-Schema

=== Controller: goals
File: server/api/goals.ts

Schema: `CreateGoalSchema` (zod) — validiert Body für POST (userId, name, target, saved, dueDate)

Exported handler: default async (event)

Funktionen / Verhalten:
- Handler(event):
  - `GET`: Validiert `userId` → ruft `GoalService.getGoalsByUserId(userId)` → gibt Liste zurück
  - `GET /api/goals/:id`: Liest `id` aus Route → ruft `GoalService.getGoalById(id)` → gibt einzelne Goal zurück
  - `POST`: Liest Body, validiert → baut `payload` (inkl. saved defaulting to 0) → ruft `GoalService.createGoal(payload)` → gibt Ergebnis zurück
  - `PUT /api/goals/:id`: Liest `id` + Body (teilweise validiert) → ruft `GoalService.updateGoal(id, data)` → gibt Ergebnis zurück
  - `DELETE /api/goals/:id`: Ruft `GoalService.deleteGoal(id)` → gibt Erfolgsmeldung zurück
    --> UserId und id seperat in zwei verschiedenen Endpunkten behandeln!
    - Andere Methoden → `405`
    - Fehlerbehandlung: Log + `createError`

Aufruft / Abhängigkeiten:
- `GoalService.getGoalsByUserId(Number(userId))`
- `GoalService.createGoal(payload)`

Rückgabewerte:
- GET: Array von Goals
- POST: Objekt { message, goal }

  Zusätzliche Endpunkte:
  - `GET /api/goals/:id` → `GoalService.getGoalById(id)` → einzelne Goal-DTO
  - `PUT /api/goals/:id` → `GoalService.updateGoal(id, data)` → { message, goal }
  - `DELETE /api/goals/:id` → `GoalService.deleteGoal(id)` → { message, id }


=== Controller: incomes
File: server/api/incomes.ts

Schema: `CreateIncomeSchema` (zod) — validiert Body für POST (userId, categoryId, source, amount, date, interval, note)

Exported handler: default async (event)

Funktionen / Verhalten:
- Handler(event):
  - `GET`: Validiert `userId` → ruft `IncomeService.getIncomesByUserId(userId)` → gibt Liste zurück
  - `GET /api/incomes/:id`: Liest `id` aus Route → ruft `IncomeService.getIncomeById(id)` → gibt einzelne Income zurück
  - `POST`: Liest Body, validiert → baut `payload` → ruft `IncomeService.createIncome(payload)` → gibt Ergebnis zurück
  - `PUT /api/incomes/:id`: Liest `id` + Body (teilweise validiert) → ruft `IncomeService.updateIncome(id, data)` → gibt Ergebnis zurück
  - `DELETE /api/incomes/:id`: Ruft `IncomeService.deleteIncome(id)` → gibt Erfolgsmeldung zurück 
    --> UserId und id seperat in zwei verschiedenen Endpunkten behandeln!
    - Andere Methoden → `405`
    - Fehlerbehandlung: Log + `createError`
...
- `IncomeService.createIncome(payload)`

Rückgabewerte:
- GET: Array von Income-Domain-Objekten (vom Service transformiert)
- POST: Objekt { message, income }

  Zusätzliche Endpunkte:
  - `GET /api/incomes/:id` → `IncomeService.getIncomeById(id)` → einzelnes Income-DTO
  - `PUT /api/incomes/:id` → `IncomeService.updateIncome(id, data)` → { message, income }
  - `DELETE /api/incomes/:id` → `IncomeService.deleteIncome(id)` → { message, id }


=== Controller: transaction
File: server/api/transactions.ts

Exported handler: default async (event) via `defineEventHandler`

Funktionen / Verhalten:
- Handler(event):
  - Liest HTTP-Methode (`getMethod`) und Query-Parameter
  - Unterstützte Methode: `GET`
    - Validiert `userId` mit `QueryUserIdSchema`
    - Ruft `TransactionService.getAllTransactions(userId)` auf
    - Gibt das Ergebnis von `TransactionService.getAllTransactions` zurück (Array formatierter Transaktionen)
  - Nicht unterstützte Methoden → `405 Method not allowed`
  - Fehler: Loggt den Fehler und returned `createError` mit StatusCode

Aufruft / Abhängigkeiten:
- `TransactionService.getAllTransactions(Number(userId))`

Rückgabewerte:
- Bei Erfolg: Array von Transaktionen (im Format, das `TransactionService` liefert)
- Bei Fehler: HTTP-Error (Status 4xx/5xx)
