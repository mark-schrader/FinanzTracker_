...
| Finanzdaten-Export |  gering | Export als CSV, Excel oder PDF zur Archivierung oder Analyse |  SE3

| Prognosen | gering | Vorhersage zukünftiger Finanzentwicklungen basierend auf bisherigen Daten | SE3
|===
Wir mussten uns dazu entscheiden, die Export- sowie die Prognosefunktion in Software Engineering 2 nicht mehr umzusetzen. Einer der Hauptgründe war die umfangreiche Umstrukturierung unseres Backends, die überraschend viel Zeit in Anspruch nahm und unseren Zeitplan verzögert hat. Während dieser Phase konnten wir hauptsächlich das Frontend weiterentwickeln und erst zu einem späteren Zeitpunkt die Verbindung zwischen Frontend und Backend herstellen.
