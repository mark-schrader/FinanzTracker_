...
| Eintragen von Finanzeinnahmen |  | Möglichkeit zur manuellen Eingabe von Einnahmen inkl. Datum, Betrag, Kategorie | xx
| Eintragen von Finanzausgaben | | Erfassung aller Ausgaben mit Tags/Kategorien und Wiederholungseinstellungen | xx
| Kategorisierung von Einnahmen und Ausgaben | | System zur Verwaltung und Anpassung von Kategorien | xx
| Übersicht über aktuelle Finanzen | | Dashboard mit grafischer und tabellarischer Darstellung aktueller Finanzlage | xx
| Prognosen | | Vorhersage zukünftiger Finanzentwicklungen basierend auf bisherigen Daten | xx
| Spar-challenges | | Vordefinierte oder benutzerdefinierte Challenges mit Fortschrittsanzeige | xx
| Wiederkehrende Ausgaben automatisch ins nächste Semester übernehmen | | Automatisches Kopieren der wiederkehrenden Ausgaben ins neue Semester (Grundkonzept) | xx
| Finanzdaten-Export | | Export als CSV, Excel oder PDF zur Archivierung oder Analyse | xx
| Simples Layout | | Intuitives und minimalistisches UI für einfache Bedienung | xx
| Zweite Sprache (Englisch) | |  Umschaltfunktion zwischen Deutsch und Englisch | xx
| Dark-Mode | | Dunkles Farbschema für angenehme Nutzung bei wenig Licht | xx
| Individuelle anpassbare Elemente | | Benutzerdefinierte Gestaltung von Farben, Diagrammen und Zeitraumfiltern | xx
| Modulares Layout (Entfernen von Modulen möglich) | | Frei anpassbares Interface, bei dem einzelne Module ein-/ausblendbar sind | xx
...
| Rechnungs -& Belegscanner | | xx
| AI-basierter Finanzcoach | | xx
| Offline-Funktionalität | | xx
| 2-Faktor-Authentifizierung | | xx
|===
